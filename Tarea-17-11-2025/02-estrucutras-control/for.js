...
// Si se usa var en la inicialización del for, la variable se elevaría (hoisting) al scope de la función o al global
// si no hay función, y sería accesible fuera del bucle, creando posibles "fugas" de variables
for (let i = 0; i < 3; i++) {
    // var multiplicacionVar = i * 2;  // Con var no hay Block Scope, se elevaría al scope superior
...
// IMPORTANTE: var NO tiene Block Scope, si se usa var en bucles anidados, las variables se elevarían
// al scope de la función o al global si no hay función, y serían accesibles fuera de los bucles
for (let i = 1; i <= 3; i++) {
    for (let j = 1; j <= 3; j++) {
        // var jVar = j;  // Con var no hay Block Scope, se elevaría al scope superior
        console.log("i = " + i + ", j = " + j);
    }
...
// Cuando la variable se declara fuera del for, tiene scope del módulo en este caso de este archivo for.js a eso se refiere con modulo
